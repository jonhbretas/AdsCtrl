        ...
        {creatives.map((c, i) => {
          ...
          return (
            <div key={c.adId || i} onClick={() => onSelect?.(c)} className={cn("grid gap-2 px-4 py-2.5 border-b border-border/30 items-center text-xs hover:bg-accent/20 transition-colors cursor-pointer", focusAds.has(c.adId) && "bg-amber-500/5")} style={{ gridTemplateColumns: "28px 2fr 80px 80px 100px 80px 70px 70px 80px 70px 80px 80px 80px 80px 1fr" }}>
              ...
              <Heat value={c.metrics.video.isVideo && c.metrics.video.hookRate != null ? c.metrics.video.hookRate : null} benchmark={BM ? BM((cr) => cr.metrics.video.hookRate) : null} sample={c.sampleStatus}>{c.metrics.video.isVideo && c.metrics.video.hookRate != null ? `${c.metrics.video.hookRate.toFixed(1)}%` : "—"}</Heat>
              <Heat value={c.metrics.video.isVideo && c.metrics.video.holdRate != null ? c.metrics.video.holdRate : null} benchmark={BM ? BM((cr) => cr.metrics.video.holdRate) : null} sample={c.sampleStatus}>{c.metrics.video.isVideo && c.metrics.video.holdRate != null ? `${c.metrics.video.holdRate.toFixed(1)}%` : "—"}</Heat>
