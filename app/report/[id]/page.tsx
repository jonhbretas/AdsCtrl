...
export default function ReportPage({ params }: { params: { id: string } }) {
    ...
    fetch("/api/accounts")
      ...
      .then((d) => {
        const acc = (d.accounts || []).find(
          (a: any) => a.account_id === params.id || `act_${a.account_id}` === params.id
        );
        setAccount(acc || null);
      })
  ...
  const name = account?.name || `Conta ${params.id}`;
  ...
  return (
    <div style={{ maxWidth: 980, margin: "0 auto", padding: "28px 24px", fontFamily: "system-ui, sans-serif", color: "#111" }}>
      ...
      <header className="report-head" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 12, color: "#888", fontWeight: 600, letterSpacing: 0.5 }}>RELATÓRIO DE MÍDIA PAGA · META</div>
      ...
      <footer style={{ marginTop: 20, fontSize: 11, color: "#bbb", textAlign: "center" }}>
        Gerado por AdsCtrl · dados ao vivo da Meta Marketing API
      </footer>
    </div>
  );
}
