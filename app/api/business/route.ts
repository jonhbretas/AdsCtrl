...
function calculateMonth(month: string, clients: any[], entries: any[], charges: any[], contracts: any[], recurring: any[]) {
  ...
  const recurringRows = revenueRows.filter((row) => row.source === "recurring" || row.recurrence === "monthly"); const activeRevenueRules = recurring.filter((rule) => rule.kind === "revenue" && rule.active); const mrr = recurringRows.length ? sum(recurringRows) : sum(activeRevenueRules);
  ...
  const clientsView = active.map((client) => { const rows = currentEntries.filter((entry) => entry.client_id === client.id); const clientRules = activeRevenueRules.filter((rule) => rule.client_id === client.id); const contract = contracts.filter((item) => item.client_id === client.id).sort((a, b) => String(b.end_date || "").localeCompare(String(a.end_date || "")))[0]; const end = contract?.end_date || client.contract_end_date; const days = end ? Math.ceil((Date.parse(`${end}T23:59:59`) - Date.now()) / 86400000) : null; const clientCharges = charges.filter((charge) => charge.client_id === client.id); return { id: client.id, name: client.name, mrr: sum(recurringRows.filter((entry) => entry.client_id === client.id)) || sum(clientRules), revenue: sum(rows.filter((entry) => entry.kind === "revenue")), received: sum(rows.filter((entry) => entry.kind === "revenue" && entry.status === "confirmed")), overdue: sum(clientCharges.filter((charge) => charge.status === "OVERDUE")), contract_end_date: end, renewal: days == null ? "not_configured" : days < 0 ? "expired" : days <= (client.contract_notice_days || 30) ? "in_notice" : "active", renewal_days: days }; });
  return { month, active_clients: active.length, new_clients: created.length, mrr, new_mrr: sum(activeRevenueRules.filter((rule) => created.some((client) => client.id === rule.client_id))), investment, cac: created.length ? investment / created.length : null, revenue, expenses, net_profit: revenue - expenses, renewal_rate: renewalRate, variable_revenue: variableRevenue, churned_clients: churned.length, lost_mrr: lostMrr, delinquency_amount: overdueCharges + overdueEntries, warning_clients: clientsView.filter((client) => client.renewal === "in_notice").length, ltv: avgRetention == null ? null : mrr * avgRetention, avg_retention_months: avgRetention, avg_time_to_churn_months: null, clients: clientsView };
}
