...
export async function POST(req: Request) {
  try {
    ...
    for (let cursor = new Date(Date.UTC(generationStart.getUTCFullYear(), generationStart.getUTCMonth(), 1)); cursor <= end; cursor.setUTCMonth(cursor.getUTCMonth() + 1)) { const due = monthDate(cursor.getUTCFullYear(), cursor.getUTCMonth(), rule.day_of_month); if (due < rule.starts_on || (rule.ends_on && due > rule.ends_on)) continue; entries.push({ client_id: rule.client_id, category_id: rule.category_id, kind: rule.kind, status: "planned", description: rule.description, amount: rule.amount, due_date: due, source: "recurring", external_id: `${data.id}:${due}`, recurrence: "monthly", notes: `Gerado pela receita recorrente ${data.id}`, updated_at: new Date().toISOString() }); }
    if (entries.length) { const { error: entriesError } = await sb.from("financial_entries").upsert(entries, { onConflict: "source,external_id" }); if (entriesError) throw entriesError; }
