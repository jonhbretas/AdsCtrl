...
export default function AppNav({ brand }: { brand?: string }) {
  ...
  return <>
    <aside className="fixed left-0 top-0 z-30 hidden h-screen w-56 flex-col border-r border-border/50 bg-sidebar md:flex">
      <div className="shrink-0 border-b border-border/50 px-3 py-3"><Link href="/" className="flex items-center gap-2.5 rounded-lg transition-opacity hover:opacity-80"><div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 text-xs font-bold text-white shadow-sm">A</div><div><div className="text-sm font-semibold text-foreground">{brandName}</div><div className="text-[10px] leading-tight text-muted-foreground">Dash · sequência da operação</div></div></Link><QuickSearch /><button type="button" onClick={() => window.dispatchEvent(new Event("adsctrl:open-ai"))} className="mt-2.5 flex w-full items-center gap-2.5 rounded-lg border border-primary/25 bg-gradient-to-r from-primary/10 to-transparent px-3 py-2 text-left text-xs font-semibold text-primary transition-all hover:border-primary/40 hover:bg-primary/15"><span className="grid h-6 w-6 place-items-center rounded-md bg-primary/15"><BrainCircuit className="h-3.5 w-3.5" /></span><span className="flex-1">Assertivus AI</span><span style={{ animationDuration: "2400ms", animationTimingFunction: "ease-in-out" }} className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" /></button></div>
    ...
    <header className="sticky top-0 z-30 flex h-12 items-center justify-between border-b border-border/50 bg-background/80 px-4 backdrop-blur-lg md:hidden"><div className="flex items-center gap-2.5"><button onClick={() => setSidebarOpen(true)} className="-ml-1 p-1 text-muted-foreground hover:text-foreground"><Menu className="h-5 w-5" /></button><Link href="/" className="flex items-center gap-2.5"><div className="flex h-6 w-6 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 text-[10px] font-bold text-white">A</div><span className="text-sm font-semibold">{brandName}</span></Link></div><div className="flex items-center gap-2"><QuickSearch compact /><DropdownMenu><DropdownMenuTrigger asChild><button className="flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-cyan-400 to-blue-500 text-[10px] font-bold text-white">A</button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48"><DropdownMenuLabel>Admin</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuItem onClick={() => setTheme(theme === "dark" ? "light" : "dark")}>{theme === "dark" ? "☀️" : "🌙"} {theme === "dark" ? "Modo claro" : "Modo escuro"}</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={logout} className="text-destructive focus:text-destructive"><LogOut className="h-4 w-4" /> Sair</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div></header>

    {sidebarOpen && <div className="fixed inset-0 z-40 bg-black/50 md:hidden" onClick={() => setSidebarOpen(false)}><aside className="absolute bottom-0 left-0 top-0 w-72 overflow-y-auto border-r border-border/50 bg-sidebar" onClick={(event) => event.stopPropagation()}><div className="flex h-12 items-center justify-between border-b border-border/50 px-4"><Link href="/" onClick={() => setSidebarOpen(false)} className="flex items-center gap-2.5"><div className="flex h-6 w-6 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 text-[10px] font-bold text-white">A</div><span className="text-sm font-semibold">{brandName}</span></Link><button onClick={() => setSidebarOpen(false)} className="p-1 text-muted-foreground"><X className="h-5 w-5" /></button></div><nav className="space-y-1 p-2">{NAV_GROUPS.map((group, index) => <div key={group.key}>{index > 0 && <div className="mx-2 my-2 h-px bg-border/50" />}<GroupBlock group={group} mobile /></div>)}<div className="my-2 h-px bg-border/50" /><button onClick={logout} className="flex w-full items-center gap-3 rounded-lg border-none bg-transparent px-3 py-2 text-sm font-medium text-destructive transition-colors hover:bg-destructive/10"><LogOut className="h-4 w-4" /> Sair</button></nav></aside></div>}
