...
export default function AppNav({ brand }: { brand?: string }) {
  ...
  return <>
    <aside className="fixed left-0 top-0 z-30 hidden h-screen w-56 flex-col border-r border-border/50 bg-sidebar md:flex">
      <div className="shrink-0 border-b border-border/50 px-3 py-3"><div className="flex items-center gap-2.5"><div className="flex h-7 w-7 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 text-xs font-bold text-white shadow-sm">A</div><div><div className="text-sm font-semibold text-foreground">{brandName}</div><div className="text-[10px] leading-tight text-muted-foreground">Dash · sequência da operação</div></div></div><QuickSearch /><button type="button" onClick={() => window.dispatchEvent(new Event("adsctrl:open-ai"))} className="mt-2.5 flex w-full items-center gap-2.5 rounded-lg border border-amber-300/20 bg-gradient-to-r from-amber-300/10 to-transparent px-3 py-2 text-left text-xs font-semibold text-amber-600 transition-all hover:border-amber-300/35 hover:bg-amber-300/15 dark:text-amber-300"><span className="grid h-6 w-6 place-items-center rounded-md bg-amber-300/15"><BrainCircuit className="h-3.5 w-3.5" /></span><span className="flex-1">Assertivus IA</span><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" /></button></div>
